...
- [ ] **Database Schema Updates** (AC: 7, 8)
  - [ ] Run migration 009_add_waiting_room_and_spotlight.sql (already created)
  - [ ] Verify waiting_room_participants table created with RLS policies
  - [ ] Verify spotlight_participant_id column added to meetings table
...
- [ ] **Gallery View Component** (AC: 2)
  ...
  - [ ] Use native scrolling for >16 participants (no virtualization needed for MVP)
  ...
  **Scrolling Strategy:**
  - Use native CSS overflow-y: auto for participant list
  - No virtualization needed for MVP (P2P WebRTC limited to 1-4 participants)
  - Gallery view optimized for <25 participants
  - Note: Virtualization with react-window deferred to Story 3.x after SFU migration
...
- [ ] **Pin/Spotlight Features** (AC: 3, 4)
  ...
  **Conflict Resolution Rule:**
  - Host spotlight (global) OVERRIDES user pin (local) for all participants
  - When spotlight active, pin button shows disabled state with tooltip: "Host has spotlighted [Name]"
  - User can pin again after host removes spotlight (spotlight_participant_id set to NULL)
  - UI priority: Spotlight indicator > Pin indicator in video tile badges
...
- [ ] **Host Participant Controls** (AC: 6)
  ...
  - [ ] Write API endpoint and visual regression tests

  **Testing:**
  - Unit tests for participant control API functions (remove, mute, promote)
  - E2E tests using Cypress with screenshot comparison:
    - Before/after participant removal
    - Gallery view layout with 2, 3, 4 participants
    - Waiting room UI states (empty, 1 participant, multiple)
  - Visual regression approach: Cypress cy.screenshot() + manual review
  - Note: Dedicated visual regression framework (Chromatic, Percy) deferred to Epic 3
...
- [ ] **Self-View Management** (AC: 9)
  ...
  - [ ] Implement draggable self-view using react-draggable (desktop only)
  ...
  **Mobile/Tablet Behavior:**
  - Self-view NOT draggable on mobile (<768px) or tablet (<1024px)
  - Fixed position system: 4 corner options (bottom-right, bottom-left, top-right, top-left)
  - Tap self-view to cycle through 4 positions
  - Desktop: Full drag capability with react-draggable
  - Rationale: Touch-based dragging is awkward, simplify for better UX
  - Note: Full mobile drag deferred to Story 2.7 (Mobile Optimization)

  **Preference Persistence:**
  - Desktop: Save drag position to localStorage + optional API sync
  - Mobile: Save corner preference to localStorage + optional API sync
  - On mount: Load from API → localStorage → default (bottom-right)
  - Uses new PATCH `/api/users/me/preferences` endpoint (see API Endpoints #8)
...
- [ ] **Immersive Mode** (AC: 10)
  ...
  - [ ] Implement exit mechanisms (ESC, button, F key)
  - [ ] Write tests for immersive mode behavior

  **Exit Mechanisms (Priority Order):**
  1. **ESC key** - Primary exit (native browser fullscreen behavior, cannot be overridden)
  2. **Hover top-right corner** - Exit button appears after 2s with fade-in animation
  3. **F key** - Toggle (consistent with entry method)

  **Implementation Notes:**
  - ESC key handled by browser's Fullscreen API automatically
  - Exit button uses absolute positioning with z-index 9999
  - F key listener only active when in immersive mode
  - All exits trigger same cleanup: document.exitFullscreen() + restore UI state
...
8. **PATCH `/api/users/me/preferences`** (Optional Enhancement)
   - Persist self-view position and size across devices
   - Zod schema:
     ```typescript
     {
       self_view_position?: { x: number; y: number };  // Pixel coordinates
       self_view_size?: 'small' | 'medium' | 'large';
       self_view_corner?: 'bottom-right' | 'bottom-left' | 'top-right' | 'top-left';  // Mobile
     }
     ```
   - User can only update own preferences
   - Stored in users.preferences JSONB column (already exists)
   - Returns: Updated user preferences object
   - Validation: Position values must be positive integers, size/corner enum validation
   - Rationale: Enhance UX by persisting preferences across browser sessions/devices
...
- **Large Participant Lists:** Use native CSS overflow-y: auto for scrolling (MVP supports 1-4 participants via P2P WebRTC, virtualization deferred to Story 3.x with SFU migration)
