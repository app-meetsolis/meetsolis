...
Solid execution against a story that was significantly de-risked by the PM precheck (A1–A6). The decision to reuse Story 6.2b's `sessions.transcript_chunks` + `/api/sessions/[id]/transcript-live` rather than ship a parallel pipeline is the right call and probably saved a day. The polling-vs-Realtime trade-off is reasonable given Clerk-Supabase JWT auth isn't currently wired — Realtime publication is in place via migration 029, dormant until that bridge lands.

UpcomingSessionRow extraction was the right move when the parent file was about to bust the 500-line limit. RowMenu, BotPill, and JoinWithNotetakerButton are appropriately co-located rather than spread further.

Type safety is good — `ManualTranscriptionProvider` and `CoachBriefWindowMinutes` are properly narrowed. Zod refine for the IN-list is correct. Render-time speaker mapping (not write-time) is the right architectural choice; it means speaker_map updates after-the-fact correctly rewrite labels without history rewrites.

### Refactoring Performed

None during this review — code quality was already acceptable.

### Compliance Check

- Coding Standards: ✓ Types in shared package, Zod on inputs, Clerk auth on routes, no direct `process.env`, no `any`
- Project Structure: ✓ Components co-located under `apps/web/src/components/{settings,dashboard}/` per A5
- Testing Strategy: ✓ Unit tests for pure helpers + API routes; integration coverage thinner (see gaps below)
- All ACs Met: ✓ Functionally complete; some ACs covered by manual QA only (live panel auto-collapse, sticky-scroll detection)

### Improvements Checklist

- [ ] **(REL-001, medium)** Surface 207 partial-failure to user — currently treated as success (`res.ok === true`) but `body.partial` flag is ignored. Add: `const body = await res.json(); if (body.partial) toast.warning('Partially saved — try again')`.
- [ ] **(TEST-001)** Add React Testing Library integration test verifying panel auto-collapses when `transcript_streaming_complete` flips to true
- [ ] **(TEST-001)** Add API test for `GET /api/sessions/active` covering: empty result, active session w/ no sessions-row yet, completed session filtered out
- [ ] **(UX-001)** Store Google Calendar `htmlLink` during sync (Story 6.1's `/api/calendar/sync`) → enables proper deep-link in row menu
- [ ] **(PERF-001)** Add exponential backoff to `useActiveSessions` poll interval when no sessions have been active for N consecutive polls
- [ ] **(EDGE-001)** Document `wasCompleteRef` reset-on-unmount behavior with an inline comment, or persist via queryClient state to avoid duplicate toast on session re-emergence

### Security Review

No new attack surface. All routes Clerk-authed via `auth()`. Zod schemas guard all PATCH bodies. Service-role bypass pattern in `/api/sessions/active` (and reused `/api/sessions/[id]/transcript-live` from 6.2b) is the existing codebase convention — manual `user_id` filter applied on every query. Defense-in-depth opportunity exists (RLS-aware client), but not a regression vs. existing routes.

### Performance Considerations

- 3s polling on `/transcript-live` matches Recall's 2-5s chunk cadence — appropriate
- 10s polling on `/sessions/active` is cheap (small query), continues indefinitely while Pro dashboard open — acceptable, future backoff suggested
- JSONB chunk append performance covered by existing `append_transcript_chunk` plpgsql function (Story 6.2b)
...
Gate: **CONCERNS** → docs/qa/gates/6.5-coach-brief-settings-bot-toggle-live-transcript.yml

Quality score: **90/100** (one CONCERNS-level NFR — reliability/partial-failure UX)

### Recommended Status

✗ **Changes Required** — but only the one medium item (REL-001 partial-failure UX). Everything else is "future" backlog and not ship-blocking.

If team prefers to ship as-is and fix REL-001 in a follow-up, mark Done + open a P2 ticket. Story owner decides final status.

---

### Follow-up — 2026-05-23 (dev)

**REL-001 fix applied.** `savePreferences` now reads the response body; if `body.partial` is true, surfaces a warning toast naming the failed table (`"Some preferences didn't save (user_preferences) — try again"`). Gate updated to **PASS** (quality score 100). Other "future" items remain as backlog suggestions, not ship-blockers.

- File: `apps/web/src/components/settings/PreferencesTab.tsx` — `savePreferences` returns `{partial, failed}` shape; `save` wrapper branches toast variant
- Tests: 24/24 still passing (no new tests added — 207 response shape already covered by route test)
- Type-check: clean
