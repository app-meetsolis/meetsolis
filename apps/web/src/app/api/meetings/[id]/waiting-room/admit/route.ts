 ...
 * Auto-joins the participant to the meeting after admission.
 */
...
export async function POST(
  ...
  try {
    ...
    const validation = AdmitRequestSchema.safeParse(body);
    ...
    if (meetingError || !meeting) {
      console.error('[Admit API] Meeting lookup error:', meetingError);
      return NextResponse.json(
    ...
    if (userError || !currentUser) {
      console.error('[Admit API] User lookup error:', userError);
      return NextResponse.json(
    ...
    if (participantError || !participant) {
      console.error('[Admit API] Participant lookup error:', participantError);
      return NextResponse.json(
    ...
    // Authorization: Only host or co-host can admit
    if (participant.role !== 'host' && participant.role !== 'co-host') {
      return NextResponse.json(
        {
          error: 'Forbidden',
          message: 'Only host or co-host can admit participants',
        },
        { status: 403 }
      );
    }
    ...
    if (waitingError || !waitingParticipant) {
      console.error(
        '[Admit API] Waiting participant lookup error:',
        waitingError
      );
      return NextResponse.json(
        {
    ...
    // Update waiting room status to admitted
    ...
    if (updateError) {
      console.error('[Admit API] Update error:', updateError);
      return NextResponse.json(
        {
          error: 'Update failed',
    ...
    // Broadcast realtime event to notify the admitted participant
    ...
    console.log('[Admit API] Participant admitted:', {
      meetingId,
      targetUserId,
    ...
    console.error('[Admit API] Unexpected error:', error);
    return NextResponse.json(
      {
