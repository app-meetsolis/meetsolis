...
describe('POST /api/billing/resume', () => {
  ...
  it('resumes subscription and returns 200', async () => {
    mockAuth.mockResolvedValue({ userId: 'clerk_1' } as ReturnType<
      typeof auth
    > extends Promise<infer T>
      ? T
    ...
    mockGetInternalUserId.mockResolvedValue('user_1');
    mockGetSupabase.mockReturnValue(
      makeSupabase({ dodo_subscription_id: 'sub_abc' }) as ReturnType<
        typeof getSupabaseServerClient
      >
    );
    const mockBilling = {
    ...
    expect(mockBilling.resumeSubscription).toHaveBeenCalledWith('sub_abc');
  });
});
